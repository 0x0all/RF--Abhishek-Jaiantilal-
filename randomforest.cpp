...
int main(){
    ...
    int cols=9, rows=32769, rows2=58921;
    
    int i,j;
    
    char X_filename[100], Z_filename[100];
    FILE *fp_X, *fp_Z;
    int p_size=cols, n_size=rows;
    
    double X[rows*cols], Y[rows], Z[rows2*cols];
    ...
    /**************READ DATA***********************/    
    strcpy(X_filename,"train_z.csv");
    strcpy(Z_filename,"test_z.csv");
        
    fp_X = fopen(X_filename,"r");
    fp_Z = fopen(Z_filename,"r");
    ...
    while (fgets(line, 1024, fp_X))
    {
        ...
        for (tok = strtok(line, ";"); tok && *tok; tok = strtok(NULL, ";\n"))
        {
            ...
            { 
               Y[i-1] = atof(tok); 
               if (DEBUG_ON) printf("%f\n", Y[i-1]);
               break;
            }
            else
            {
               X[(i-1)*cols+j] = atof(tok);
            }
            j++;
        }
        i++;   
    ...
    long end_ = (long)time(NULL);
    printf("time (sec.): %ld\n", end_ - start_);
    
    // printf("memory: %ld\n", memory_end - memory_start);
    
    FILE *fp = fopen("res_.csv", "wb");
    if (fp != NULL)
    {
        for(i=0;i<rows2;i++) fprintf(fp, "%f\n", ypred[i]);
        fclose(fp);
    }
